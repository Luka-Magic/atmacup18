...
class Atma18Dataset(Dataset):
    def __init__(self,
        ...
        self.traffic_light_generator = traffic_light_generator
    ...
    def __getitem__(self, idx):
        ...
        if self.traffic_light_generator is not None:
            ...
            img = np.concatenate([img, traffic_image], axis=0)
...
def get_dataloader(cfg, df, traffic_light_generator, feat_columns, phase):
    if phase == 'train':
        dataset = Atma18Dataset(
            df,
            feat_columns,
            transform=get_transforms(cfg, phase),
            traffic_light_generator=traffic_light_generator
        )
    ...
    if cfg.train:
        for fold in cfg.use_folds:
            ...
            if cfg.use_traffic_light:
                traffic_light_generator = TrafficLightMaskGenerator(image_size=[cfg.img_h, cfg.img_w])
            else:
                traffic_light_generator = None

            train_dl = get_dataloader(cfg, train_df, traffic_light_generator, feat_columns, 'train')
            valid_dl = get_dataloader(cfg, valid_df, traffic_light_generator, feat_columns, 'valid')
