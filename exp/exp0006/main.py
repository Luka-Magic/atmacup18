...
from generators import TrafficLightMaskGenerator
    ...
    def __getitem__(self, idx):
        ...
        if self.use_depth:
            ...
            for postfix in ['-1.0', '-0.5', '']:
                ...
                depth = depth / 255.0
                depths.append(depth)
            depths = np.concatenate(depths, axis=2)
        ...
        if self.use_vehicle_segmentation:
            vehicle_segmentations = []
            for postfix in ['-1.0', '-0.5', '']:
                vehicle_segmentation = np.array(Image.open(str(self.vehicle_segmentation_dir / row['ID'] / f'image_t{postfix}.png')))
                vehicle_segmentation = cv2.resize(vehicle_segmentation, (self.img_w, self.img_h))
                vehicle_segmentation = vehicle_segmentation.astype(np.float32)[:, :, np.newaxis]
                vehicle_segmentation = vehicle_segmentation / 255.0
                vehicle_segmentations.append(vehicle_segmentation)
            vehicle_segmentations = np.concatenate(vehicle_segmentations, axis=2)
            vehicle_segmentations = vehicle_segmentations.transpose(2, 0, 1)
            vehicle_segmentations = torch.tensor(vehicle_segmentations)
...
def get_dataloader(cfg, df, phase):
    if phase == 'train':
...
def main(cfg):
    ...
    if cfg.train:
        for fold in cfg.use_folds:
            ...
            train_dl = get_dataloader(cfg, train_df, 'train')
            valid_dl = get_dataloader(cfg, valid_df, 'valid')
            ...
            # torchinfo
            torchinfo.summary(model, input_size=(cfg.train_bs, img_in_chans, cfg.img_h, cfg.img_w))
