...
class Atma18Dataset(Dataset):
    def __init__(self, df, feat_columns=None, transform=None, return_label=True):
        super().__init__()
        self.df = df
...
def get_dataloader(cfg, df, feat_columns, phase):
    if phase == 'train':
        dataset = Atma18Dataset(df, feat_columns, transform=get_transforms(cfg, phase))
...
def main(cfg):
    ...
    if cfg.train:
        for fold in cfg.use_folds:
            ...
            # torchinfo
            torchinfo.summary(model, input_size=[(cfg.train_bs, cfg.img_in_chans, cfg.img_h, cfg.img_w), (cfg.train_bs, len(feat_columns))])
    ...
    if oof_cfg.oof:
        ...
        for fold in cfg.use_folds:
            ...
            valid_ds = Atma18Dataset(valid_feat_df, feat_columns=feat_columns, transform=get_transforms(cfg, 'valid'), return_label=False)
            valid_dl = DataLoader(
                valid_ds,
                batch_size=cfg.valid_bs,
                shuffle=False,
    ...
    if test_cfg.test:
        ...
        assert len(test_cfg.use_folds) == len(test_cfg.model_weight), f"len(test_cfg.use_folds) must be equal to len(test_cfg.model_weight)"

        if not all([(SAVE_DIR / f'fold{fold}_best.pth').exists() for fold in test_cfg.use_folds]):
            print("Not all models exist")
            return

        test_preds = []
        for i, fold in enumerate(test_cfg.use_folds):
            model = Atma18Model(
                name=cfg.model_name,
                img_in_chans=cfg.img_in_chans,
                output_dim=cfg.output_dim,
            ...
            test_ds = Atma18Dataset(test_df, feat_columns, transform=get_transforms(cfg, 'valid'), return_label=False)
            test_dl = DataLoader(
                test_ds,
                batch_size=cfg.valid_bs,
                shuffle=False,
