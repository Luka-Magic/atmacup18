    ...
    if cfg.train:
        ...
        for fold in range(cfg.n_folds):
            ...
            wandb.init(
                ...
                entity='luka-magic',
                name=GBDT_ID if not cfg.debug else f'{GBDT_ID}_debug',
            ...
            eval_func = eval('mae')
            score_fold = eval_func(y.loc[valid_indices].values, oof_predictions[valid_indices])
            print(f'fold: {fold}, score: {score_fold:<.4f}')
            wandb.log({f'score_fold{fold}': score_fold})
        ...
        pred_columns = [f'pred_{i}' for i in range(len(TARGET_COLUMNS))]
